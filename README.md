...
这个仓库是给 AI 编程助手执行的，不要求你先读懂代码。把 [INSTALL_PROMPT.md](INSTALL_PROMPT.md) 整段复制给 Claude Code 或 Codex，填好“我的目标”，AI 会先判断 Windows 或 Mac，再安装 Skill、检查微信版本、处理降级和更新防护，最后完成验证并按你的目标查询。

CTA：**把你的本地微信记录交给 Codex，变成可检索、可整理的个人资料库；不上传原始聊天，不需要你手动研究密钥和数据库。**

如果你只想直接开始，也可以把这句话发给 AI：

> 请安装并使用这个仓库的 WeChat Chat Decrypt Skill，只处理我本人电脑上已登录的微信数据；先完成版本检查、备份提醒、密钥提取和 SQLite 验证，再按我的目标查询，不要上传或打印任何密钥和聊天原文。
